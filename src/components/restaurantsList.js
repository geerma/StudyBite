...
const RestaurantsList =()=>{
    ...
    const genJsx= (obj,id)=>{
        ...
        return (
            <li key={id} id={id} onClick={handleClick}>{obj.name}</li>
        )
    }
    useEffect(()=>{
        const getData = async () => {
    ...
    return(
        ...
        <div className={styles.darkBg} onClick={handleClick}>
            ...
            <RestaurantCard name={restaurant.name} restId={restId} rating={rating} hours={"5:30pm"} description={restaurant.restaurantDescription} waiting={18}/>
          </div>
        </div>
        :""
        }
